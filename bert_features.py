...
class Bert(object):
	...
	def __init__(self, pretrained_model, layer_indexes, max_seq_length, batch_size, do_lower_case=False, which_cuda = 0):
		...
		self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model, do_lower_case=do_lower_case) 
	...
	def extract_bert_features(self, sentences):
		...
		for input_ids, input_mask, token_maps, example_indices in eval_dataloader:
			...
			all_encoder_layers = all_encoder_layers[2][1:]
			#print(all_encoder_layers,len(all_encoder_layers))
			#print(all_encoder_layers[0].shape,all_encoder_layers[1].shape,all_encoder_layers[2].shape)
